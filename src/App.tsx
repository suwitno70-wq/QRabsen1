...
export default function App() {
  ...
  const handleRoleSwitch = (newRole: UserRole) => {
    if (newRole === 'ADMIN') {
      ...
      handleLogin({
        id: guru?.id || 'GURU-01',
        username: guru?.username || 'ahmad',
        guruId: guru?.id || 'GURU-01',
        name: guru?.nama || 'Ahmad Fauzi, S.Pd.I',
        role: 'GURU',
        nip: guru?.nip || '198506122010011018',
  ...
  if (!currentUser) {
    return (
      <div className="font-sans antialiased text-slate-800 selection:bg-emerald-200">
        <LoginView onLogin={handleLogin} guruList={guruList} />
            ...
            <>
              ...
              {(activeTab === 'teachers' || activeTab === 'guru') && (
                <MasterGuru
                  ...
                  mapelList={mapelList}
                  onSaveGuru={(g) => {
                    AppStorage.saveGuru(g);
                    addToast('success', 'Data guru diperbarui');
                  }}
                  onDeleteGuru={(id) => {
                    AppStorage.deleteGuru(id);
                    addToast('info', 'Data guru dihapus');
                  }}
                  onImportGuru={(imported) => {
                    imported.forEach((g) => AppStorage.saveGuru(g));
            ...
            <>
              ...
              {(activeTab === 'admin_guru' || activeTab === 'guru') && (
                <MasterGuru
                  ...
                  onSaveGuru={(g) => {
                    AppStorage.saveGuru(g);
                    addToast('success', 'Data guru berhasil disimpan');
                  }}
                  onDeleteGuru={(id) => {
                    AppStorage.deleteGuru(id);
                    addToast('info', 'Data guru dihapus');
              ...
              {(activeTab === 'admin_kelas' || activeTab === 'kelas') && (
                <MasterKelas
                  ...
                  guruList={guruList}
                  onSaveKelas={(k) => {
                    AppStorage.saveKelas(k);
                    addToast('success', 'Data kelas berhasil disimpan');
                  }}
                  onDeleteKelas={(id) => {
                    AppStorage.deleteKelas(id);
                    addToast('info', 'Data kelas dihapus');
                  }}
                  onPreviewQR={(k) => {
                  ...
                  onOpenQRPosters={() => setActiveTab('admin_posters')}
                  onImportKelas={(imported) => {
                    imported.forEach((k) => AppStorage.saveKelas(k));
              ...
              {(activeTab === 'admin_mapel' || activeTab === 'mapel') && (
                <MasterMapel
                  mapelList={mapelList}
                  onSaveMapel={(m) => {
                    AppStorage.saveMapel(m);
                    addToast('success', 'Data mapel disimpan');
                  }}
                  onDeleteMapel={(id) => {
                    AppStorage.deleteMapel(id);
                    addToast('info', 'Data mapel dihapus');
                  }}
                  onImportMapel={(imported) => {
                    imported.forEach((m) => AppStorage.saveMapel(m));
              ...
              {(activeTab === 'admin_jadwal' || activeTab === 'jadwal') && (
                <MasterJadwal
                  ...
                  mapelList={mapelList}
                  onSaveJadwal={(j) => {
                    AppStorage.saveJadwal(j);
                    addToast('success', 'Jadwal mengajar disimpan');
                  }}
                  onDeleteJadwal={(id) => {
                    AppStorage.deleteJadwal(id);
                    addToast('info', 'Jadwal dihapus');
                  }}
                  onImportJadwal={(imported) => {
                    imported.forEach((j) => AppStorage.saveJadwal(j));
