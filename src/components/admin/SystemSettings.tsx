...
export const SystemSettings: React.FC<SystemSettingsProps> = ({
  ...
  const handleResetDatabase = async () => {
    const confirmText = prompt(
      'PERINGATAN: Seluruh data (Guru, Kelas, Mapel, Jadwal, Presensi) akan disetel ulang ke data bawaan awal.\n\nKetik "RESET" untuk melanjutkan:'
    );
    if (confirmText === 'RESET') {
      await AppStorage.resetToSampleData();
      setBackupMsg({ text: 'Database telah disetel ulang ke data standar.', success: true });
      if (onResetData) onResetData();
      setTimeout(() => setBackupMsg(null), 4000);
    }
  };
        ...
        <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-xs space-y-5 text-xs">
          ...
          <div className="p-4 rounded-2xl bg-rose-50 border border-rose-200 space-y-2">
            ...
            <p className="text-rose-700 text-[11px] leading-relaxed">
              Jika Anda ingin membersihkan seluruh data dan mengembalikan sistem ke data awal standar madrasah, gunakan tombol di bawah.
            </p>
            <div className="pt-2">
              <button
                type="button"
              ...
              >
                ...
                <span>Setel Ulang Data ke Bawaan (Reset)</span>
              </button>
            </div>
          </div>
        </div>
