...
export const MasterMapel: React.FC<MasterMapelProps> = ({
  ...
  const handleOpenAdd = () => {
    setFormData({
      id: `MP-${String(mapelList.length + 1).padStart(2, '0')}`,
      kode: '',
      nama: '',
      kelompok: 'Umum',
    });
  ...
  const handleImportSubmit = () => {
    setImportStatus('');
    if (!csvInput.trim()) {
      setImportStatus('Silakan tempel teks CSV.');
      return;
    }

    const parsed = parseMapelCSV(csvInput);
    if (parsed.length === 0) {
      setImportStatus('Gagal membaca data mapel. Pastikan format: Kode,Nama Mapel,Kelompok');
      return;
    }
            ...
            <div>
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    onClick={() => {
                      if (confirm(`Hapus mapel ${mapel.nama}?`)) {
                        onDeleteMapel(mapel.id);
                      }
                    }}
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-3 text-xs">
              <div>
                ...
                <input
                  ...
                  placeholder="Contoh: IPA, MTK, PAI, QURDIS"
                  className="w-full px-3 py-2 rounded-xl border border-slate-300 focus:border-emerald-500 font-mono font-bold uppercase outline-none"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Contoh: Al-Qur'an Hadits, IPAS & Sains"
              ...
              <div>
                ...
                >
                  ...
                  <option value="Agama">Pendidikan Agama Islam (PAI)</option>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl border border-slate-200 overflow-hidden">
            ...
            <div className="p-5 space-y-3 text-xs">
              <div>
                <label className="block font-bold text-slate-700 mb-1">Tempel Format CSV (Kode,Nama,Kelompok):</label>
                <textarea
                  rows={5}
                  value={csvInput}
                  onChange={(e) => setCsvInput(e.target.value)}
                  placeholder={`Kode,Nama Mata Pelajaran,Kelompok\nIPA,Ilmu Pengetahuan Alam,Umum\nAA,Akidah Akhlak,Agama\nBD,Bahasa Daerah,Muatan Lokal`}
