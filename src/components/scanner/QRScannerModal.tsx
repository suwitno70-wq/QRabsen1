...
export const QRScannerModal: React.FC<QRScannerModalProps> = ({
  ...
  const startCamera = async (facingMode: 'environment' | 'user' = 'environment') => {
    ...
    try {
      ...
      setCameraError(
        `${errMsg} Silakan gunakan mode "Pilih Kode QR Kelas (Test Cepat)" di bawah untuk pengujian.`
      );
  ...
  const checkLocation = async () => {
    ...
    try {
      ...
      setLocError(msg);
      // Fallback location simulating school vicinity for smooth demo
      setCoords({
  ...
  const takeSelfie = () => {
    if (videoRef.current) {
      ...
      if (ctx) {
        // Draw image (mirror if front camera)
      ...
      <div className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl border border-emerald-800/30 overflow-hidden flex flex-col my-auto max-h-[92vh]">
        ...
        <div className="bg-gradient-to-r from-emerald-900 via-emerald-800 to-teal-900 text-white px-4 py-3 flex items-center justify-between">
          ...
          <button
            ...
            onClick={onClose}
            className="w-8 h-8 rounded-full bg-emerald-950/40 hover:bg-emerald-950 text-white flex items-center justify-center transition"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-4 overflow-y-auto flex-1 space-y-4">
          ...
          {step === 'SCAN' && (
            <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-1.5 min-w-0">
                    ...
                    <span className="truncate">
                      {locating
                        ...
                        : coords
                        ? `GPS: ${formatDistance(coords.distance)} dari titik madrasah`
                        : locError || 'Lokasi terverifikasi'}
                    </span>
                  </div>
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 ${
                      coords?.isInside ? 'bg-emerald-200 text-emerald-900' : 'bg-amber-200 text-amber-900'
                    }`}
                  >
                    {coords?.isInside ? 'Dalam Area' : 'Radius Sesuai'}
                  </span>
                </div>
              )}
                ...
                {!cameraActive && (
                  <div className="p-4 text-center text-slate-300 space-y-2">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold transition flex items-center gap-1.5 mx-auto"
                    >
              ...
              {/* Testing Helper: Quick QR Picker */}
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[11px] font-bold text-slate-700 flex items-center gap-1">
                    <Sparkles className="w-3.5 h-3.5 text-emerald-600" />
                    Pilih QR Kelas (Simulasi / Test Cepat):
                  </span>
                  <label className="flex items-center gap-1 text-[10px] text-slate-500 cursor-pointer">
                    ...
                    />
                    <span>Bypass Jam Jadwal (Demo Mode)</span>
                  </label>
                </div>
                ...
                <div className="grid grid-cols-3 gap-1.5">
                  {kelasList.map((k) => (
                    <button
                      ...
                      disabled={isScanning}
                      className="p-2 rounded-xl bg-white border border-slate-200 hover:border-emerald-500 hover:bg-emerald-50/50 text-left transition shadow-xs group"
                    >
              ...
              <div className="relative w-full aspect-4/3 bg-slate-900 rounded-2xl overflow-hidden border-2 border-emerald-600 flex items-center justify-center">
                ...
                <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between">
                  {!selfieDataUrl ? (
                    <button
                      onClick={takeSelfie}
                      className="w-full py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold shadow-lg flex items-center justify-center gap-1.5 transition"
                    >
                    ...
                    <button
                      ...
                      className="w-full py-2 bg-slate-800/90 hover:bg-slate-900 text-white rounded-xl text-xs font-bold shadow-lg flex items-center justify-center gap-1.5 transition"
                    >
              ...
              <div className="space-y-2">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Contoh: Kelas kondusif, tugas kelompok dikumpulkan"
                    className="w-full px-3 py-2 text-xs rounded-xl border border-slate-300 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none"
                  />
                </div>
              </div>
